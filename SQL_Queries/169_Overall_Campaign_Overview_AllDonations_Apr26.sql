...
select distinct c.CampaignSentDate, cm.MarketingID, cm.CampaignName, cm.CampignTouchPointName as CampaignTouchPointName, 			
	...
	case when datediff(day, convert(varchar(10), c.CampaignSentDate, 110), convert(varchar(10), fct.CollectionDateSK, 110)) <= 7		
		and datediff(day, convert(varchar(10), c.CampaignSentDate, 110), convert(varchar(10), fct.CollectionDateSK, 110)) >= 0	
		then 1 else 0 end as DonatedWithon7Days	
...
left join INTEGRATION.dbo.Int_dimcampaignmaster_HT cm on cm.CampaignMastersk = c.CampaignMastersk			
left join [INTEGRATION].[dbo].[INT_MKT_FCTCampaignDonationDetail_HT] fct on fct.RSAPersonID = c.donorexternalkey			
where c.campaignsentdate >= '2018-03-01' and c.campaignsentdate < '2018-04-01'			
...
select distinct r.PersonID, 			
	...
	case when (r.MIN_REGISTRATION_DATE >= '2018-03-01' and r.MIN_REGISTRATION_DATE < '2018-04-01')		
...
where c.campaignsentdate >= '2018-03-01' and c.campaignsentdate < '2018-04-01'			
and fct.CollectionDateSK >= 20180301 and fct.CollectionDateSK < 20180408			
