...
Left Join stage.dbo.STG_HemaCenterdetail cd ON cd.centerid=dm.centerid									
	  ...
	  --LEFT OUTER JOIN integration.dbo.VW_INT_DIMEmployee ON acc.RecruiterID=VW_INT_DIMEmployee.EmpRSAID AND VW_INT_DIMEmployee.EndDate IS NULL								
	  left outer join STG_Hemapeople ppl on acc.RecruiterID = ppl.PersonID								
where import=0									
and cast(FromDateTime as date) between '2017-01-01' and '2018-07-10'																
group by									
qcr.DescShort ,									
mkt.DriveID ,									
TeamLeadEmpID,									
...
reg.Region,									
 ...
 ,reg.DriveName,reg.AccountRepName,reg.AccountInternalName,									
 DriveDate,DriveStartime,DriveEndTime									
 -- ,[Before Drive Start Def],[All Day Def Except Last Hour], [Last Hour Def], [Last 30 Min Def],[After Drive End Def]									
...
--					from Stage.[dbo].[STG_RSA_DonorDeferral] def 				
--									where def.registration_id =  mkt.REGISTRATIONID
